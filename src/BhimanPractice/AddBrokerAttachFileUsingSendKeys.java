...
public class AddBrokerAttachFileUsingSendKeys {
	
	public void fillBrokersFormAndClickSubmit() {
...
// Scrolling window
		...
		res.sendKeys(basePath + "\\UploadFiles\\UploadResumeForChrome.exe");
		
		WebElement agreem = driver.findElement(By.id("agreement"));
		action.moveToElement(agreem).build().perform();
		agreem.sendKeys(basePath + "\\UploadFiles\\UploadAgreementForChrome.exe");
		
		WebElement kc = driver.findElement(By.id("kyc"));
		action.moveToElement(kc).build().perform();
		kc.sendKeys(basePath + "\\UploadFiles\\UploadKYCForChrome.exe");
...
//		driver.close();
	...
	public static void main(String[] args) {
		new AddBrokerAttachFileUsingSendKeys().fillBrokersFormAndClickSubmit();
